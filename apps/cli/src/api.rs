//! Client HTTP del backend Kunuk (doc 12). Sincrono (ureq): la CLI è un flusso lineare.
//!
//! Tratta i corpi come opachi: invia/riceve JSON, non interpreta la crittografia (che vive
//! nel crypto-core, SR-1). In dev l'ingress è Caddy con CA interna (doc 07): si passa il
//! certificato radice via PEM perché la verifica TLS resti attiva (niente verifica disabilitata).

use ureq::tls::{Certificate, PemItem, RootCerts, TlsConfig};
use ureq::Agent;

/// Risposta grezza: stato HTTP e corpo come testo (i corpi dell'API sono JSON o vuoti).
pub struct Resp {
    pub status: u16,
    pub body: String,
}
...
impl Resp {
    /// Decodifica il corpo JSON in un [`serde_json::Value`]. Corpo non-JSON → errore.
...
/// Client verso una base URL (es. `https://localhost`). Riusa una sola connessione.
pub struct Client {
    agent: Agent,
    base: String,
}
...
impl Client {
    /// Costruisce il client. `ca_pem`, se presente, aggiunge una CA radice di fiducia (la CA
    /// interna di Caddy in dev): la verifica TLS resta attiva. Senza, usa i root di sistema.
    ...
    /// POST JSON, con Bearer opzionale.
    pub fn post_json(
        &self,
        path: &str,
        bearer: Option<&str>,
